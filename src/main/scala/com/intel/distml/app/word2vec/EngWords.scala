...
object EngWords {
  ...
  def normalizeString(src : String) : String = src.filter( c => ((c >= 'a') && (c <= 'z')) || ((c >= 'A') && (c <= 'Z')) || (c == ' ')).toLowerCase

  def exchange[T, U](w : (T, U)) : (U, T) = (w._2, w._1)

  def freqWordsOnly(minFreq : Int)(W : (Long, String)) : Boolean = W._1 > minFreq
  ...
  def main(args: Array[String]) {
    ...
    var totalWords = 0L
    for (i <- 0 until countedWords.length) {
      val item = countedWords(i)
      wordMap.put(item._1, i)
      totalWords += item._2
    }
    val wordTree = Word2VecModel.createBinaryTree(lineCount.toLong, countedWords)
